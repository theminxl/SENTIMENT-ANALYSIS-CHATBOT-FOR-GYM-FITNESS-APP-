...
function buildGpt2Prompt(prompt, context, title) {
    return compactText([
...
function localGpt2Fallback(prompt, context) {
    const topic = domainQuery(prompt);
    return compactText(`For ${topic}, the trained fitness knowledge points to this answer: ${context} Keep the reply practical: connect the review signal to one gym, diet, workout app, or recovery action, and avoid treating general coaching as medical advice.`, 700);
}
async function callHostedGpt2(prompt, context, title) {
    if (!hfToken)
        return null;
    ...
    const generated = Array.isArray(data) && typeof data[0]?.generated_text === "string"
...
async function openFoodFacts(query) {
...
function buildSummary(query, sources) {
    if (!sources.length) {
...
function gpt2Middleware() {
    return async (req, res, next) => {
        ...
        const context = compactText(body.context ?? "", 1200);
